...
--유효성 : 사용자로부터 값을 입력받을 때 정확안 값만 입력되도록 하는 것
--
-- 1. 개체 무결성
...
--
-- 1. 요구사항 분석
-- 고객과의 의사소톡ㅇ을 통해 고객의 업무 프로세스를 완벽하게 이해하고
...
--
-- 4. 논리 모델링
-- 최종적으로 데이터를 관리할 데이터베이스를 선택하고, 선택한 데이터베이스에 실제 테이블을 시작적인 구조를 만들었으면 그것을 실제로 SQL코딩을 통해 완성하는 단계
 ...
 * -----------------------
 * USER_PW : VARCHA2(100)
 * USER_NAME : VARCHA2(200)
 * USER_ADDRESS : VARCHA2(300)
 * USER_EMAIL : VARCHA2(300)
 * USER_BIRTH : DATE
 * 
 * 
 * PRODUCT
 ...
 * PRODUCT_PRICE : NUMVER
 * PRODUCT_COUNT : NUMVER
 * 
 * 
 * ORDER
 * ORDER_NUM : NUMVER
...
--요구사항--
-- 꽃 테이블이랑 화분테이블 두개가 필요하고 꽃을 구매할 때 화분도 같이 구매한다.
-- 꽃 테이블에는 꽃이름, 색깔, 가격이 있다.
-- 화분테이블에는 제품번호, 화분색깔, 화분모양, 꽃 이름이 있다.
...
-- 사원테이블에서 사번, 이름, 직종, 보너스비율, 실제 보너스 금액 출력
SELECT EMPPLOYEE_ID, 
	   FIRST_NAME, JOB_ID, 
	   SALARY,
	   COMMISSION_PCT, SALARY * COMMISION_PCT "BONUS"
FROM EMPPLOYEE;
...
--키워드는 대소문자를 가리지 않지만, 데이터는 대소문자를 정확하게 입력해야한다.
SELECT EMPLOYEE_ID, FIRST_NAME
FROM EMPLOYEES
WHERE FIRST_NAME = 'Michael'; 


--사원데이블에서 직종이 'IT PROG'인 사원들의 정보를 사번, 이름, 직종, 급여 순으로 출력
SELECT EMPLOYEE_ID, FIRST_NAME JOB_ID, SALARY
FROM EMPLOYEES
WHERE JOB_ID = 'IT PROG'; 


--사원테이블에서 급여가 10000 이상 그리고 13000이하인 사람의 정보를 이름,급여 순으로 조회하세요
SELECT FIRST_NAME, SALARY FROM EMPLOYEES
WHERE SALARY >= 10000 AND SALARY <= 13000;


--사원 테이블에서 입사일이 05년 9월 21일인 사원의 정보를 사번, 이름, 입사일 순으로 출력
